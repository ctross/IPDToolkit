...
#' @export
 ...
 GTFT = function(i, coop, intent, coop_error, arb, arb_error, error_rate, arb_error_rate_type_1, arb_error_rate_type_2, standing, xi=0, generosity_p=0.2){
  if(i==1){
  # Coop without error on first move
  result = c(1, 1, 0, 0, 0, standing, standing)
  }else{
  do_coop = ifelse(coop[i-1]==1,1,coin(generosity_p))
